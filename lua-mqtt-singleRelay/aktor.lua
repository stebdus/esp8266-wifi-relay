...
local function mqtt_start()
  ...
  -- MQTT msg: wifiRelay/LivingRoomLight
  -- MQTT payload: ON/OFF
      ...
      if(topic == mqtt_topic) then
        if data ~= nil then
          --print("data: "..data)
            ...
            if (data == "ON") then
              -- msg recieved to switch light on
              --print("Switch light on")
              relay1_state = RELAY_STATE_ON
              mqtt:publish(mqtt_topic_state, "ON", 0, 0)
            elseif (data == "OFF") then
              -- msg recieved to switch light off
              --print("Switch light off")
              relay1_state = RELAY_STATE_OFF
              mqtt:publish(mqtt_topic_state, "OFF", 0, 0)
            end
          end
        end
